 ...
 * its connections to the external transceiver operates without issue. The
 * example will execute multiple iterations, each iteration will do the
 * following: 1) Start the CAN driver 2) Transmit and receive 100 messages using
 * self reception request 3) Stop the CAN driver
 */
...
#define MSG_ID 0x001     // 11 bit standard format ID
...
static void can_transmit_task(void *arg) {
    can_message_t tx_msg = {.data_length_code = 1,
                            .identifier = MSG_ID,
    ...
    for (int iter = 0; iter < NO_OF_ITERS; iter++) {
        ...
        for (int i = 0; i < NO_OF_MSGS; i++) {
            ...
            ESP_LOGI(EXAMPLE_TAG, "Msg transmit - Data = %d",
                tx_msg.data[0]);
...
static void can_receive_task(void *arg) {
    ...
    for (int iter = 0; iter < NO_OF_ITERS; iter++) {
        ...
        for (int i = 0; i < NO_OF_MSGS; i++) {
            ...
            ESP_LOGI(EXAMPLE_TAG, "Msg received - Data = %d",
                     rx_message.data[0]);
        }
...
static void can_control_task(void *arg) {
    ...
    for (int iter = 0; iter < NO_OF_ITERS; iter++) {
        ...
        xSemaphoreTake(
            ctrl_sem,
            portMAX_DELAY); // Wait for TX and RX tasks to finish iteration
...
void app_main() {
    ...
    xTaskCreatePinnedToCore(can_control_task, "CAN_ctrl", 4096, NULL,
                            CTRL_TSK_PRIO, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(can_receive_task, "CAN_rx", 4096, NULL,
                            RX_TASK_PRIO, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(can_transmit_task, "CAN_tx", 4096, NULL,
                            TX_TASK_PRIO, NULL, tskNO_AFFINITY);
