...
*/
#define EXAMPLE_WIFI_SSID "escape_gmAP"
    ...
    while (1) {
        ...
        while(1){
            ...
            if (sock != -1) {
                ...
                if(shutdown(sock, 0) < 0) ESP_LOGE(TAG, "Failed shutdown()");
                if(close(sock) < 0) ESP_LOGE(TAG, "Failed close()");
                if(shutdown(listen_sock, 0) < 0) ESP_LOGE(TAG, "Failed shutdown() l");
                if(close(listen_sock) < 0) ESP_LOGE(TAG, "Failed close() l");
